...
CoupVariantCState::CoupVariantCState() {
  current_player = 0;
...
bool CoupVariantCState::is_terminal() const {
  if (action_history.size() == 0) return false;
  return action_history.back() == LOSE_CARD;
}

bool CoupVariantCState::is_chance_node() const {
  return action_history.size() == 0;
}

Player CoupVariantCState::get_current_player() const { return current_player; }

std::vector<Action> CoupVariantCState::get_legal_actions() const {
...
std::unique_ptr<CoupVariantCState> CoupVariantCState::get_child(
    Action action) const {
  auto new_state = std::make_unique<CoupVariantCState>(*this);
...
std::string CoupVariantCState::get_information_set_key() const {
  std::stringstream ss;
...
std::vector<double> CoupVariantCState::get_utilities() const {
