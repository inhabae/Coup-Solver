...
std::vector<Action> GameState::get_legal_actions() const {
    // Game start
    if (history.size() == 0) return {INCOME, FOREIGN_AID, TAX};
    ...
    bool current_player_was_assassinate_blocked = (current_player == 0) ? p1_num_assassinate_blocked >= 1: p2_num_assassinate_blocked >= 1;
    bool current_player_was_steal_blocked = (current_player == 0) ? p1_num_steal_blocked >= 1 : p2_num_steal_blocked >= 1;
    bool current_player_was_fa_blocked = (current_player == 0) ? p1_num_fa_blocked >= 1 : p2_num_fa_blocked >= 1;
    
    // vs Income, Pass_block
    if (last_action == INCOME || last_action == PASS_BLOCK) {
        if (current_player_coins >= COIN_TO_COUP) return {COUP};
...
std::string GameState::get_infoset_string() const {
    ...
    else if (current_card == AMBASSADOR) infoset_str += "AMBASSADOR: ";
