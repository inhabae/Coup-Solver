...
inline size_t get_information_set_hash(const RPSState& state) {
    ...
    for (size_t s = 0; s < state.action_history.size(); ++s) hash_combine(h, static_cast<int>(1)); // In order to hide action
    return h;
}
...
private:
    ...
    void precompute(const RPSState& state) {
        ...
        if (state.is_chance_node()) {
            std::vector<Card> C{CARD1, CARD2, CARD3};
...
public:
    ...
    void build_infosets_and_reach(
        ...
        if (cur == br) {
            for (auto a : actions) {
                auto child_ptr = state.get_child(a);
                auto& ch = *dynamic_cast<const RPSState*>(child_ptr.get());
                build_infosets_and_reach(ch, br, opp_strat, r);
            }
        } else {
...
int main() {
    ...
    solver.train_with_exploitability_tracking(root, 1000000, 1000, 0.01);
