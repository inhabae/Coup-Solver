...
std::string get_action_name(Action action) {
    ...
    return it != ACTION_NAMES.end() ? it->second : "Unknown action " + std::to_string(static_cast<int>(action));
}
...
int main() {
    std::cout << "Loading strategies from strategies.bin\n" << std::string(50, '-') << "\n";
    try {
        auto strategies = load_strategies_binary("strategies.bin");
        std::cout << "Dictionary Contents (" << strategies.size() << " information sets):\n" << std::string(50, '-') << "\n";
        ...
        std::cerr << "Error: " << e.what() << "\nEnsure 'strategies.bin' exists and matches the expected format.\n";
        return 1;
    }
    return 0;
}