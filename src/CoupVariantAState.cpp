...
bool CoupVariantAState::is_terminal() const {
    ...
    return action_history.back() == LOSE_CARD; // Game is only over after "Action 8 = Lose card"
}

bool CoupVariantAState::is_chance_node() const {
    return action_history.size() == 0; // Game is only at a chance node for initial card dealings, where action history is empty
}

Player CoupVariantAState::get_current_player() const {
...
std::vector<Action> CoupVariantAState::get_legal_actions() const {
    ...
    if (last_action == INCOME || last_action == PASS_BLOCK || last_action == GAME_START) {
        if (current_player_coins >= 2) return {COUP};
        if (current_player_coins == 1) {
            if (current_player == 0 && !did_assassinate_p1) {
                return {INCOME, ASSASSINATE};
            } else if (current_player == 1 && !did_assassinate_p2) {
                return {INCOME, ASSASSINATE};
            }
            return {INCOME};
        }
        return {INCOME}; // current_player_coins < 2
...
std::unique_ptr<CoupVariantAState> CoupVariantAState::get_child(Action action) const {
    auto new_state = std::make_unique<CoupVariantAState>(*this); // Copy state
    ...
    if (action == INCOME) {
        ...
        if (current_player == 0) {
            new_state->player1_coins = player1_coins - 1;
            new_state->did_assassinate_p1 = true;
        }  else if (current_player == 1) {
            new_state->player2_coins = player2_coins - 1;
            new_state->did_assassinate_p2 = true;
        }
        return new_state;
    } 

    return new_state; // Other actions do not change the game states, just switch turns
}
...
std::string CoupVariantAState::get_information_set_key() const {
    ...
    std::cerr << "[ERROR] get_infomratino_set_key() does not return." << std::endl;
    std::exit(1);
}
