...
inline size_t get_information_set_hash(const CoupVariantAState& state) {
    size_t h = 0;
...
inline size_t get_history_hash(const CoupVariantAState& state) {
    size_t h = 0;
...
private:
    ...
    std::unordered_map<size_t, std::vector<CoupVariantAState>> iset_states;
    ...
    InformationSet* get_iset(const CoupVariantAState& state) {
    ...
    void precompute(const CoupVariantAState& state) {
        if (state.is_terminal()) return;
        if (state.is_chance_node()) {
            std::vector<Card> C{ASSASSIN, CONTESSA, CIVILIAN};
            for (auto c1 : C) for (auto c2 : C) if (c1 != c2) {
...
public:
    ...
    void initialize(const CoupVariantAState& root) {
    ...
    double calculate_exploitability(const CoupVariantAState& root) {
    ...
    void build_infosets_and_reach(
        ...
        if (cur == br) {
            for (auto a : actions) {
                auto child_ptr = state.get_child(a);
                auto& ch = *dynamic_cast<const CoupVariantAState*>(child_ptr.get());
                build_infosets_and_reach(ch, br, opp_strat, r);
            }
        } else {
    ...
    double calculate_best_response(
        ...
        if (state.is_chance_node()) {
            double exp_u = 0.0;
            std::vector<Card> cards = {ASSASSIN, CONTESSA, CIVILIAN};
            for (auto c1 : cards) for (auto c2 : cards) if (c1 != c2) {
                    CoupVariantAState child = state;
                    child.player1_card = c1;
                    child.player2_card = c2;
                    child.action_history.push_back(GAME_START);
...
int main() {
    CoupVariantAState root;
    CFRPlusSolver solver(100);
    solver.initialize(root);
    solver.train_with_exploitability_tracking(root, 1000000, 10000, 0.01);
